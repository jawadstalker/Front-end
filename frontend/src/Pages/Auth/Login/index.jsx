        ...
        <Box sx={{ display: { xs: "none", md: "flex" }, flexDirection: "column", justifyContent: "space-between", p: { md: 5, lg: 6 }, background: "linear-gradient(145deg, #EFF6FF 0%, #EEF2FF 52%, #ECFEFF 100%)", borderLeft: "1px solid #E5E7EB" }}>
          <Box>
            ...
            <Stack spacing={1.3} sx={{ mt: 4 }}>
              {features.map((item) => <Box key={item.title} sx={{ display: "flex", alignItems: "center", gap: 1.7, p: 1.4, borderRadius: 3, background: "rgba(255,255,255,.72)", border: "1px solid rgba(148,163,184,.18)", transition: "all .2s ease", "&:hover": { transform: "translateX(-3px)", boxShadow: "0 8px 24px rgba(37,99,235,.08)" } }}>
                <Box sx={{ width: 40, height: 40, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 2.2, background: "#DBEAFE", color: colors.primary }}>{React.cloneElement(item.icon, { sx: { fontSize: 20 } })}</Box>
                <Box><Typography sx={{ color: colors.text, fontSize: 12, fontWeight: 800 }}>{item.title}</Typography><Typography sx={{ color: colors.muted, fontSize: 10, mt: .35 }}>{item.text}</Typography></Box>
              </Box>)}
            </Stack>
          </Box>
